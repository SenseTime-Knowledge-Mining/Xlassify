...
def start_rdp(train_index, valid_index, fold, seed):
    print(f'Fold:{fold}')
    ...
    # P = (counts+0.5)/(len(train_y)+1) #
  
    # P_V_G[i][j] = P(j|i) j∈V,i∈G  命名方式模仿sklearn.naive_bayes.MultinomialNB
    # Pi = (np.sum(kmer_X, axis=0)+0.5)/(len(train_y)+1)
    Pi = 0.5 # (n(wi)+0.5)/(N+1) but not have corpus
    feature_count_ = np.dot(train_Y.T, train_X)
    class_count_ = train_Y.sum(axis=0) # 每个genus的样本个数
    smoothed_fc = feature_count_ + Pi
    P_V_G = smoothed_fc/(class_count_[:,None]+1)
    pred_Y= np.dot(valid_X, np.log(P_V_G.T))
...
if __name__ == '__main__':
    ...
    for seed in kfold_seeds:
        ...
        for train_index,test_index in kf.split(item_df.index, item_df["label_name"]):
            ...
            pred_tmp = start_rdp(train_index, test_index, fold, seed)
